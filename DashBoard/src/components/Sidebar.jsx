...
const Sidebar = () => {
  const {darkMode, setDarkMode} = myCustomContext();
    ...
    <>
      <div className="flex flex-col items-center py-4 border-r border-black w-1/5 ">
        ...
        <div className="flex mt-6 gap-4 w-full pl-5">
          <div onClick={handleClick} className="w-10 h-10 bg-white border-2 border-black rounded-lg cursor-pointer"></div>
          <div onClick={handleClick} className="w-10 h-10 bg-black border-2 border-black rounded-lg cursor-pointer"></div>
        </div>
      </div>
    </>
  );
